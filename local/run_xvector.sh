...
required="spk2utt utt2num_frames feats.scp"
...
if [ $stage -le 6 ]; then
  ...
  awk -v id=0 '{print $1, id++}' $data/spk2utt > $egs_dir/spk2int
  utils/sym2int.pl -f 2 $egs_dir/spk2int $data/utt2spk > $egs_dir/utt2int
fi
...
if [ $stage -le 7 ]; then
  ...
  $cmd --max-jobs-run $nj JOB=1:$total_nj $egs_dir/tmp/filter_utt2int.JOB.log \
    utils/filter_scp.pl $egs_dir/feats.JOB.scp $egs_dir/utt2int \>$egs_dir/utt2int.JOB || exit 1;

  for i in `seq $total_nj`; do
    echo "$egs_dir/feats.$i.ark $egs_dir/utt2int.$i"
  done > $egs_dir/feat_id_ark.list
fi

if [ $stage -le 8 ]; then
  num_spks=$(awk '{print $2}' $egs_dir/utt2int | sort | uniq -c | wc -l)
  num_frames=$(awk '{n += $2} END {print n}' <$egs_dir/utt2num_frames)
  num_epochs=$[($num_frames * $num_repeats) / $frames_per_epoch + 1]

  echo $num_spks >$egs_dir/num_spks
  ...
  ./local/train_xvector.py --seed 0611 \
    --num_classes $num_spks \
